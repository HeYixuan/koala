...
public class MyInvocationSecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
    ...
    public void init() {
        ...
        Collection<SystemRole> roles = systemRoleClient.getList();
        //应当是资源为key， 权限为value。 资源通常为url， 权限就是那些以ROLE_为前缀的角色。 一个资源可以由多个权限来访问。
        resourceMap = new ConcurrentHashMap<>();
        roles.forEach(role -> {
            ConfigAttribute ca = new SecurityConfig(role.getRoleAlias());
            Collection<SystemMenu> resources = systemMenuClient.loadByRole(role.getId());
            ...
            if ("ROLE_ANON".equals(role.getRoleAlias())) {
                Collection<SystemMenu> unbindResources = systemMenuClient.loadUnbound();
